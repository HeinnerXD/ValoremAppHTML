sap.ui.define([
	...
	"use strict";
	return Controller.extend("valoremapp.ValoremAppHTML.controller.PlaneacionFinanciera.PlaneacionFinancieraJunta", {
		onInit: function () {
			this.oRouter = sap.ui.core.UIComponent.getRouterFor(this.getView());
            this.oRouter.getRoute("PlaneacionFinancieraJunta").attachMatched(this._onRouteMatched, this);
		},
        ...
        onDowload: function (filename) {
            ...
            $.post(urlServer + "/api/downloadFileRoot", {
                ...
                url: "01. TENANT VALOREM/01. VALOREM/05. PLANEACION FINANCIERA/03. Producción/01. Informe Junta/ADMINISTRACIÓN",
                fileName: filename
            }).done(function (data) {
        ...
        onDowloadReports: function (filename) {
            var urlServer = "https://valoremserverdev.cfapps.us10.hana.ondemand.com";
            var oView = this.getView();
            var oResourceBundle = oView.getModel("i18n").getResourceBundle();
            var url = oResourceBundle.getText("urlServer").toString().trim();
            ...
            $.post(urlServer + "/api/downloadFileRoot", {
                name: "DMSShuttle",
                username: "lgarcia@valorem.com.co",
                url: "01. TENANT VALOREM/01. VALOREM/05. PLANEACION FINANCIERA/03. Producción/01. Informe Junta/CARGA Y REPORTES",
                fileName: filename
            }).done(function (data) {
